...
async def test_node_repository_crud(
    ...
    node = await repo.create(NodeCreate(title="Idea"), OWNER)
    assert (await repo.get(node.id, OWNER)).title == "Idea"

    updated = await repo.update(node.id, NodeUpdate(title="Renamed"), OWNER)
    assert updated.title == "Renamed"

    await repo.delete(node.id, OWNER)
    with pytest.raises(NodeNotFoundError):
        await repo.get(node.id, OWNER)
...
async def test_node_repository_pagination(
    ...
    repo = NodeRepository(store, embedder)
    for i in range(7):
        await repo.create(NodeCreate(title=f"Idea {i}"), OWNER)

    page = await repo.list_paginated(OWNER, skip=0, limit=3)
    assert page.total == 7
    assert len(page.items) == 3
    assert page.skip == 0
    assert page.limit == 3

    next_page = await repo.list_paginated(OWNER, skip=3, limit=3)
    assert len(next_page.items) == 3
    assert {n.id for n in page.items}.isdisjoint({n.id for n in next_page.items})

    last_page = await repo.list_paginated(OWNER, skip=6, limit=3)
...
async def test_edge_repository_pagination(
    ...
    nodes = [await node_repo.create(NodeCreate(title=str(i)), OWNER) for i in range(3)]
    for n in nodes[:-1]:
        await edge_repo.create(EdgeCreate(source_id=n.id, target_id=nodes[-1].id), OWNER)

    page = await edge_repo.list_paginated(OWNER, skip=0, limit=1)
    assert page.total == 2
...
async def test_node_repository_computes_embedding_on_create(
    ...
    repo = NodeRepository(store, embedder)
    node = await repo.create(NodeCreate(title="Idea", content="details"), OWNER)
...
async def test_node_repository_recomputes_embedding_when_content_changes(
    ...
    repo = NodeRepository(store, embedder)
    node = await repo.create(NodeCreate(title="Idea", content="v1"), OWNER)
    original_embedding = node.embedding

    updated = await repo.update(node.id, NodeUpdate(content="v2"), OWNER)
...
async def test_node_repository_keeps_embedding_when_unrelated_field_changes(
    ...
    updated = await repo.update(node.id, NodeUpdate(color="#000000"), OWNER)
